    ...
    <style>
        ...
        div.row div.clear-shortcode-xs-6 {
         ...
         @media (min-width: 1200px){
             div.row div.clear-shortcode-xs-6:nth-child(4n+5){
                clear: left;
             }
          }
                ...
                if (count($files) > 0) {
                    ...
                    foreach ($files as $i => $filename) {
                        ...
                        while (($line = fgets($handle)) !== false) {  
                            ...
                            // if (preg_match("/\.(" . ( (basename($filename, ".css") == "font-awesome")  ? "fa" : ".") . "-[\w\d_-]+)\:before\s*\{/i", $line, $result)) {
                            //     array_push($icons, $result[1]);  
                            // }
                            ...
                            switch (basename($filename, ".css")) {
                                ...
                                case  "hotel-pictograms":
                                    if(preg_match("/\.(material-design-[\w\d_-]+)\:before\s*\{/i", $line, $result)) {
                                        array_push($icons, $result[1]);  
                                    }
                                    if(preg_match("/\.(flaticon-[\w\d_-]+)\:before\s*\{/i", $line, $result)) {
                                        array_push($icons, $result[1]);  
                                    }                            
                                    break;                           
                        ...
                        foreach ($icons as $i => $value) {
                            if (fmod($i + $bp, $bp) == 0) {
                                ...
                                echo '<div class="col-lg-3 col-md-4 col-sm-6 col-xs-6 clear-shortcode-xs-6">';
                            }
